...
if ($text == "🧑‍💻 سرویس های من" && $step == "none") {
    ...
    foreach ($userServers as $server) {
        $financialItems .= "📌 <code>$server</code>\n\n";
    }
    $theText = ($myServers != "") ? "🗂 لیست سرور های خریداری شده شما به شرح زیر می باشد :\n$myServers" : "⚠️ خطا : شما هنوز هیچ سروری خریداری نکردید. اگر هزینه سرور را واریز کردید و هنوز آن را دریافت نکردید، مدتی صبر کنید. همکاران ما در سریع ترین زمان سرور شمارا ساخته و از طریق ربات برای شما ارسال می کنند.";
    SendMessage($chat_id, $theText, "HTML", $message_id);
}
//---------- Free Test ----------
if ($text == "🔥تست رایگان" && $step == "none") {
    $theText = ($testServer != "") ? "📍 سرور زیر، جهت تست کیفیت و سرعت سرویس های ما در دسترس شما کاربران عزیز قرار داده شده است :\n$testServer" : "❌ درحال حاضر هیچ سرویسی برای تست رایگان قرار نگرفته است❌";
    SendMessage($chat_id, $theText, "HTML", $message_id);
}
//---------- Support Section ----------
